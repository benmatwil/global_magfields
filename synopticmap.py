...
def get_pole_value(date, pole=None, average=15):
  ...
  datelist = [dt.datetime(int(date[:4]), int(date[4:6]), int(date[6:8])) + dt.timedelta(days=x) for x in xrange(-15, 15)]
...
def pole_filler(map, start, fillvalue):
  ...
  allnan = False
  for k in xrange(ilat-dir, start-dir, -dir):
    row[:] = map[:,k]
...
def smooth_map(map):
  ...
  for ilon in xrange(-2,map.shape[0]-2):
    for ilat in xrange(-2,map.shape[1]-2):
